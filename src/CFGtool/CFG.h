 ...
 *
 * Defines the Control Flow Graph (CFG) for a routine. It extends the common 
 * PrivateCFG class implementation with functionality specifically needed for
 * a CFG profiler.
 */

#ifndef _MIAMI_CFGTOOL_CFG_H
#define _MIAMI_CFGTOOL_CFG_H
...
public:
  ...
  //------------------------------------------------------------------------------------------------------------------
   ...
   inline Node* CfgEntry() const { return (static_cast<CFG::Node*>(cfg_entry)); }
   inline Node* CfgExit() const  { return (static_cast<CFG::Node*>(cfg_exit)); }
   ...
   void SetEdgeCounts(LoadModule *myimg, const MIAMI_CFG::count_type *thr_counters);
