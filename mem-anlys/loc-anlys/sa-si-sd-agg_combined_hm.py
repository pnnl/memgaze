    ...
    for j in range(0, len(arRegionId)):
       ...
       # To show all three in tandem
        ...
        if (len(cols_df_SP) > 3 and len(cols_df_SI) >3 and len(cols_df_SD)>3):
            ...
            gs0_ax_0 = sns.heatmap(df_hm_SD, cmap='mako_r',cbar=True, annot=False,ax=gs0_ax_0,vmin=0.02,vmax=1.0)
            gs0_ax_1 = sns.heatmap(df_hm_SP, cmap='mako_r',cbar=True, annot=False,ax=gs0_ax_1,vmin=0.02, vmax=1.0)
            ...
            i=0
            for ticklabel, tickcolor in zip(gs0_ax_2.get_xticklabels(), my_colors):
