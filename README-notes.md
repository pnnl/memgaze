...
* Bugs:
  - The trace generated by memgaze-xtrace (perf-script) looses the connection between IPs and load modules. If two load modules binaries are instrumented for the same program, memgaze-analyze cannot correctly process one of them. Also, memgaze-analyze should read all load-classification files.
