...
then
  ...
  echo "./compile.sh <FOLDER> <BIN> <ARGS> <0(time)/1(load)> <BufferSIZE(def=8192)> <PERIOD(def=37500000)> <Mask(Rec=0xffffffffffff)>"
...
then
  SIZE=$5
  PERIOD=$6
  time ./collect_data.sh ${FOLDER}/${BIN} """$ARGS""" ${SIZE} ${PERIOD} 0 ${LOAD}
  ...
  rm ${FOLDER}/${BIN}_s${SIZE}_p${PERIOD}.trace
  rm ${FOLDER}/${BIN}_s${SIZE}_p${PERIOD}.trace.clean
  Period=$PERIOD
  if [ ${LOAD} -eq 0 ]
  then
  ...
  time ./execute.sh ${FOLDER}/${BIN}_PTW.hpcstruct ${FOLDER}/${BIN}.lc_Fixed ${FOLDER}/${BIN}_s${SIZE}_p${PERIOD}.trace.final $Period $LOAD
fi
if [ "$#" -eq 7 ]
then
  FUNC=$7
  SIZE=$5
  PERIOD=$6
  time ./collect_data.sh ${FOLDER}/${BIN} """$ARGS""" ${SIZE} ${PERIOD} 0 ${LOAD} ${CURR} ${FUNC}
#  time ./collect_data.sh ${FOLDER}/${BIN} """$ARGS""" ${SIZE} ${PERIOD} 0 ${LOAD} 
  mv ${FOLDER}/${BIN}.data ${FOLDER}/${BIN}_s${SIZE}_p${PERIOD}.data
  time ./extract_data.sh ${FOLDER}/${BIN}_s${SIZE}_p${PERIOD}
  time ./removeErros.sh ${FOLDER}/${BIN}_s${SIZE}_p${PERIOD}.trace
  ...
  time ./execute.sh ${FOLDER}/${BIN}_PTW.hpcstruct ${FOLDER}/${BIN}.lc_Fixed ${FOLDER}/${BIN}_s${SIZE}_p${PERIOD}.trace.final $Period $LOAD $FUNC
fi
if [ "$#" -eq 3 ]
then
  ...
  rm ${FOLDER}/${BIN}_full.trace
  rm ${FOLDER}/${BIN}_full.trace.clean
